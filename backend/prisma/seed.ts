...
const channelSeeds: { slug: string; name: string; topic: string; lines: string[] }[] = [
  {
    ...
    lines: [
      ...
      "Shipped the responsive shell tweaks — **please** sanity-check on narrow widths.",
      ...
      "I'll verify preview height at 800px and 1200px tonight.",
  ...
  },
  {
    ...
    lines: [
      "Exploring a calmer default theme for nested portfolio views — thoughts?",
      ...
      "Do we show member count in header on narrow mobile widths?",
      ...
      "Mobile-first pass deferred — portfolio shell priority.",
  ...
  },
  {
    ...
    lines: [
      ...
      "Reminder: do **not** use `helmet.frameguard` — blocks embedding.",
      ...
      "Verify dicebear svg loads when nested — CORS OK.",
  ...
  },
  {
    ...
    lines: [
      "Career page preview height plan: 720px default — aligns with spec.",
      ...
      "Verify ⌘K quick switcher focus traps when nested.",
      ...
      "Confirm websocket works when loaded from the production parent URL.",
      "Double-check no `100vh` in layout — use `height:100%` chain.",
      "Modals must portal to app root — not parent document.",
      "I'll walk through the embed smoke page in browser MCP session.",
      ...
      "I'll capture 800px vs 1200px layout screenshots.",
      "QA: image lightbox closes on Esc within the shell.",
      ...
      "Reminder: JWT comment in README about localStorage tradeoff.",
      "Verify the sample embed's `allow` attribute includes clipboard.",
...
async function main() {
  ...
  const dmBodies = [
    ...
    "Linked in `#design` pin. Worry about 900px width.",
